      ...
      <div class="modal-content">
        ...
        <form @submit.prevent="handleSubmit" class="product-form">
          ...
          <div v-if="showAddCategoryForm" class="add-category-form">
            ...
            <button type="button" @click="addNewCategory" class="btn btn-small">
              確認新增
            </button>
            <button
              type="button"
          ...
          <div v-if="productForm.imageUrl.length > 0" class="image-preview">
            <div
              v-for="(image, index) in productForm.imageUrl"
              :key="index"
              class="image-item"
            >
              <img
      ...
      <tbody>
        ...
        >
          ...
          <td>
            ...
            <button
              @click="deleteProduct(product.storeProductId)"
...
const handleSubmit = async () => {
  ...
  try {
    ...
    if (productForm.newImages && productForm.newImages.length > 0) {
      const validFiles = productForm.newImages.filter(
    ...
    if (editingProduct.value) {
      response = await storeServices.updateStoreProduct(
        editingProduct.value.storeProductId,
        formData
      );
    } else {
...
const addNewCategory = async () => {
  ...
  try {
    const response = await storeServices.createCategory({
...
const removeImage = (index: number) => {
  ...
  if (removedItem instanceof File) {
    productForm.newImages = productForm.newImages.filter(
      (file) => file !== removedItem
    );
  } else {
    productForm.originalImages = productForm.originalImages.filter(
      (url) => url !== removedItem
    );
  }
};
